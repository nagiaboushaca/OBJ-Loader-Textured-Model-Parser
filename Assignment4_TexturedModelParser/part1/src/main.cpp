...
// boolean for toggling wireframe
...
* @return void
*/
void VertexSpecification(OBJ myObj){

	// We will load a texture here prior
	gTexture.LoadTexture(myObj.getPathToPPM());
	...
	const std::vector<GLfloat> vertexData = myObj.getVertices();
    ...
    const std::vector<GLuint> indexBufferData = myObj.getIndices();
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 indexBufferData.size()*sizeof(GL_UNSIGNED_INT),
...
void Draw(OBJ myObj){
  ...
  //Render data
	...
	if (drawWireframe) {
		...
		glDrawElements(GL_TRIANGLES, myObj.getIndices().size(), GL_UNSIGNED_INT, nullptr);
	} else {
